...
def generate_video_report(
    ...
    Args:
        breakdown_data: 分镜拆解的完整结果数据（由 breakdown_get_result 返回）
    ...
    # 构建分镜概览部分
    segments_section = _build_segments_overview(
        breakdown_data.get("segments", [])
    )
...
def _build_segments_overview(segments: list) -> str:
    ...
    for seg in segments[:10]:  # 最多显示前10个分镜
