...
def video_upload_to_tos(
    ...
    上传成功后，返回的 URL 可直接用于 breakdown_submit 工具提交分镜任务。
    ...
    if bucket_name is None:
        bucket_name = getenv("TOS_BUCKET", DEFAULT_BUCKET)
    region = getenv("TOS_REGION", DEFAULT_REGION)
    ...
    if not access_key or not secret_key:
        return {
            "error": "缺少 TOS 访问凭证。请在 config.yaml 中配置 volcengine.access_key 和 volcengine.secret_key"
        }
    ...
    try:
        ...
        return {
            ...
            "message": "视频上传成功！可以使用返回的 video_url 调用 breakdown_submit 进行分镜分析",
        }
