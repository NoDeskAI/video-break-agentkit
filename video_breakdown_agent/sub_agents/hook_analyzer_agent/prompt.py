...
例如在文本中明确写出：`视觉冲击力: 7.2/10`、`语言钩子: 6.5/10`、`综合评分: 6.8/10` 等。
分数是分析的核心输出，不能省略。
...
2. <建议2>
3. <建议3>
3) 分数字段范围固定 0-10；**必须从定性描述中推断合理分数，不要默认给 0**。
   - 如果输入中有明确的数字分数（如"视觉冲击力: 7.2/10"），直接使用该分数。
   - 如果输入中只有定性描述（如"视觉聚焦良好"、"节奏流畅"），根据描述强度推断分数：
     * 正面描述（优点、良好、流畅、优秀等）→ 推断分数 ≥ 5.0
     * 中性描述（一般、尚可等）→ 推断分数 4.0-6.0
     * 负面描述（缺乏、不足、较弱等）→ 推断分数 2.0-4.0
     * 强烈负面描述（很差、完全缺失等）→ 推断分数 0.0-2.0
   - 只有**完全没有相关描述**时才给 0。
4) 禁止输出任何函数调用片段、工具日志、系统占位串（例如 PLHD/transfer_to_agent）。
