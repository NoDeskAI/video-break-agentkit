...
class VideoPrompt(BaseModel):
    ...
    negative_prompt: str = Field(
        default="模糊，扭曲，低质量，失焦，噪点，水印",
        description="负向提示词"
    )
    
    # 参考图片
    first_frame: Optional[str] = Field(default=None, description="参考首帧图片URL或base64")
    last_frame: Optional[str] = Field(default=None, description="参考尾帧图片URL或base64")
    ...
    # 原始分镜数据（用于评估对比）
    original_segment_data: Optional[Dict] = Field(default=None, description="原始分镜数据")
...
class VideoPromptList(BaseModel):
    ...
    transfer_target: Optional[str] = Field(default=None, description="迁移目标（如：手机壳产品）")
...
class VideoGenerationResult(BaseModel):
    ...
    # 拼接结果
    merged_video_url: Optional[str] = Field(default=None, description="拼接后的完整视频URL")
    merged_video_path: Optional[str] = Field(default=None, description="拼接后的本地路径")
