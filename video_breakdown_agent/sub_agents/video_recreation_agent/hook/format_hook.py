    ...
    *,
    ...
    try:
        ...
        if "total_selected" not in parsed:
            parsed["total_selected"] = sum(1 for p in parsed["prompts"] if p.get("selected", False))
        
        if "total_cost" not in parsed:
            parsed["total_cost"] = sum(p.get("estimated_cost", 0) for p in parsed["prompts"] if p.get("selected", False))
        
        # 重新序列化并更新响应
        llm_response.content.parts[0].text = json.dumps(parsed, ensure_ascii=False, indent=2)
        return llm_response
        ...
        if json_match:
            try:
                ...
                llm_response.content.parts[0].text = json.dumps(parsed, ensure_ascii=False)
                return llm_response
            except:
                pass
    ...
    *,
    ...
    try:
        ...
        if "total_requested" not in parsed:
            parsed["total_requested"] = len(parsed["generated_videos"]) + len(parsed["error_list"])
