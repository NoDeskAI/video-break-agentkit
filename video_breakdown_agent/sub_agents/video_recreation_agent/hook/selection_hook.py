...
def hook_segment_selection(
    ...
    try:
        ...
        if match1:
            ...
            # 替换中文标点和"和"
            segments_str = segments_str.replace('，', ',').replace('、', ',').replace('和', ',').replace(' ', '')
            segment_indices = [int(x.strip()) for x in segments_str.split(',') if x.strip().isdigit()]
        ...
        if selection_updated:
            ...
            pending_prompts["total_selected"] = sum(1 for p in prompts if p.get("selected", False))
            pending_prompts["total_cost"] = sum(p.get("estimated_cost", 0) for p in prompts if p.get("selected", False))
            pending_prompts["total_duration"] = sum(p.get("duration", 0) for p in prompts if p.get("selected", False))
            
            session.state["pending_prompts"] = pending_prompts
            
            logger.info(f"分镜选择已更新: 选中{pending_prompts['total_selected']}/{len(prompts)}个")
