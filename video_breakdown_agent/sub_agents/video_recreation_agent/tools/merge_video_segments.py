    ...
    try:
        ...
        if isinstance(generated_videos, str):
            ...
            for idx, item in enumerate(generated_videos):
                if isinstance(item, str):
                    normalized.append({f"segment_{idx+1}": item, "segment_index": idx + 1})
        ...
        if len(generated_videos) == 1:
            ...
            for key, value in video_data.items():
                if key != "segment_index" and isinstance(value, str) and value.startswith("http"):
                    video_url = value
                    break
        ...
        # 按segment_index排序
        sorted_videos = sorted(generated_videos, key=lambda x: x.get("segment_index", 0) if isinstance(x, dict) else 0)
        ...
        for idx, video_data in enumerate(sorted_videos):
            ...
            segment_name = list(video_data.keys())[0] if isinstance(video_data, dict) and len(video_data) > 1 else f"segment_{idx+1}"
            video_url = video_data.get(segment_name) if isinstance(video_data, dict) else video_data
        ...
        if len(downloaded_files) < len(generated_videos):
            logger.warning(f"部分视频下载失败：{len(downloaded_files)}/{len(generated_videos)}")
        ...
        tool_context.state["recreated_video_url"] = f"file://{output_path}"  # 临时使用本地路径
