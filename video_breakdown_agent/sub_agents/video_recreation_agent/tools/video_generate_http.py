...
async def _upload_base64_to_tos(base64_url: str) -> Optional[str]:
    ...
    try:
        ...
        bucket = getenv("DATABASE_TOS_BUCKET") or getenv("TOS_BUCKET", "video-breakdown-uploads")
        ...
        try:
            ...
            logger.info(f"[_upload_base64_to_tos] base64 图片已上传到 TOS: {object_key}")
...
def validate_video_model_config() -> tuple[bool, str]:
    ...
    """
    # 检查API Key
    api_key = getenv("MODEL_VIDEO_API_KEY", getenv("MODEL_AGENT_API_KEY", settings.model.api_key))
    ...
    **kwargs
    ...
    if first_frame_image:
        ...
        if _is_base64_data_url(first_frame_image):
            logger.info("[generate_single_video] 检测到 base64 首帧图片，正在上传到 TOS...")
            first_frame_image = await _upload_base64_to_tos(first_frame_image)
            if not first_frame_image:
                logger.warning("[generate_single_video] base64 首帧图片上传失败，将跳过首帧图片")
            else:
    ...
    if last_frame_image:
        ...
        if _is_base64_data_url(last_frame_image):
            logger.info("[generate_single_video] 检测到 base64 尾帧图片，正在上传到 TOS...")
            last_frame_image = await _upload_base64_to_tos(last_frame_image)
            if not last_frame_image:
                logger.warning("[generate_single_video] base64 尾帧图片上传失败，将跳过尾帧图片")
            else:
    ...
    if reference_images:
        ...
        for ref_img in reference_images:
            ...
            if _is_base64_data_url(ref_img):
                logger.info("[generate_single_video] 检测到 base64 参考图片，正在上传到 TOS...")
                ref_img = await _upload_base64_to_tos(ref_img)
                if not ref_img:
                    logger.warning("[generate_single_video] base64 参考图片上传失败，将跳过该图片")
                    continue
                else:
                    logger.info("[generate_single_video] base64 参考图片已转换为公开 URL")
        ...
        async with session.post(
            ...
            if response.status != 200:
                ...
                try:
                    ...
                    error_msg = error_json.get("error", {}).get("message") or error_json.get("message")
                except Exception:
                    # 如果 JSON 解析失败，读取文本
                    try:
                ...
                logger.error(
                    f"视频生成请求失败: HTTP {response.status}, "
                    f"错误信息: {error_msg}"
                )
            ...
            try:
                async with session.get(
                    ...
                    if status == "succeeded":
                        ...
                        # 仍在处理中
                        logger.debug(f"任务{task_id}当前状态: {status}, 已等待{elapsed_time}s")
...
async def video_generate(
    ...
    try:
        ...
        for p in selected_prompts:
            logger.info(f"  - {p.get('segment_name', 'unknown')}: {p.get('duration', 5)}秒, 提示词长度={len(p.get('positive_prompt', ''))}")
        ...
        await asyncio.gather(*[submit_task(item) for item in selected_prompts], return_exceptions=True)
        
        logger.info(f"📤 任务提交完成: 成功{len(task_dict)}个, 失败{len(error_list)}个")
        
        # 第2步：轮询任务状态（并发轮询）
        logger.info(f"⏳ 开始轮询任务状态（共{len(task_dict)}个任务，每10秒查询，最多10分钟）...")
        ...
        if not isinstance(success_list, list):
            logger.error(f"⚠️ success_list 格式异常: {type(success_list)}，已重置为空列表")
            success_list = []
        
        # 确保每个元素都是字典
        validated_list = [item for item in success_list if isinstance(item, dict)]
        if len(validated_list) != len(success_list):
            logger.warning(f"⚠️ success_list 中有 {len(success_list) - len(validated_list)} 个非字典元素被过滤")
        
        tool_context.state["generated_videos"] = validated_list
        logger.info(f"📦 存储 generated_videos: {len(validated_list)} 个分镜, 数据类型: {[type(x).__name__ for x in validated_list]}")
        ...
        for item in error_list:
            logger.info(f"  ❌ {item.get('segment_name', 'unknown')}: {item.get('error', 'Unknown')}")
        logger.info("=" * 60)
