...
def _resolve_frame_url(
    ...
    if url:
        logger.info(f"[_resolve_frame_url] 检测到无效 URL（占位符）：{url[:60]}，将从 session state 回查")
    else:
        logger.info(f"[_resolve_frame_url] first_frame_url 为空，将从 session state 回查 segment {segment_index} 的帧")
    ...
    logger.warning(f"[_resolve_frame_url] 未找到 segment_index={segment_index} 的分镜数据，退化为 t2v")
    return None
...
def parse_segment_info(user_message: str) -> Dict:
    ...
    segment_match = re.search(r'(?:分镜|segment|第)[\s]*(\d+)', user_message, re.IGNORECASE)
    if segment_match:
    ...
    # 解析正向提示词
    positive_match = re.search(r'正向提示词[：:](.*?)(?=负向提示词|生成方式|$)', user_message, re.DOTALL | re.IGNORECASE)
    if positive_match:
        info["positive_prompt"] = positive_match.group(1).strip()
    
    # 解析负向提示词
    negative_match = re.search(r'负向提示词[：:](.*?)(?=生成方式|预估|$)', user_message, re.DOTALL | re.IGNORECASE)
    if negative_match:
...
async def direct_video_generation(
    ...
    try:
        ...
        final_negative = negative_prompt or parsed_info["negative_prompt"] or "模糊画面、不连贯、质量差"
        ...
        logger.info(f"解析结果: {final_segment_name}，时长={final_duration}s，提示词长度={len(final_positive)}")
        ...
        if resolved_frame:
            logger.info(f"[direct_video_generation] 首帧已解析，将使用 i2v 模式")
        else:
            logger.info(f"[direct_video_generation] 无可用首帧，将使用 t2v 模式")
