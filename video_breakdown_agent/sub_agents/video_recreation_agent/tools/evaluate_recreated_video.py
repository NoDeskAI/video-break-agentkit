...
def calculate_similarity(original_data: Dict, recreated_data: Dict) -> float:
    ...
    if len(original_segments) == len(recreated_segments):
        ...
        # 部分匹配
        ratio = min(len(recreated_segments), len(original_segments)) / max(len(recreated_segments), len(original_segments))
    ...
    if original_duration > 0 and recreated_duration > 0:
        duration_ratio = min(original_duration, recreated_duration) / max(original_duration, recreated_duration)
    ...
    similarity_score += weights["visual_content"] * 0.7  # 默认70%相似（因为是基于同一提示词生成）
    ...
    try:
        ...
        if not original_vision_result:
            ...
            original_data = {
                ...
                "total_duration": sum(s.get("duration", 0) for s in original_vision_result.get("segments", [])),
        ...
        # 基于分镜数量
        if recreated_data.get("segment_count", 0) == original_data.get("segment_count", 0):
