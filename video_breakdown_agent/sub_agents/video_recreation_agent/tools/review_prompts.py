...
def format_prompt_preview(prompts: List[Dict]) -> str:
    ...
    total_cost = sum(p.get("estimated_cost", 0) for p in prompts if p.get("selected", False))
    total_duration = sum(p.get("duration", 0) for p in prompts if p.get("selected", False))
    ...
    for prompt_data in prompts:
        ...
        highlight = " - **默认选中（前3秒黄金钩子）** ⭐" if segment_index == 1 and selected else ""
        
        markdown_parts.append(f"### {checkbox} 分镜{segment_index} ({start_time:.1f}-{end_time:.1f}秒){highlight}\n\n")
        ...
        # 元数据
        markdown_parts.append(f"**预估时长**: {duration:.1f}秒 | **预估费用**: ¥{estimated_cost:.2f}元\n\n")
...
async def review_prompts(tool_context: ToolContext) -> Dict:
    ...
    try:
        ...
        logger.info(f"当前选中{selected_count}/{total_count}个分镜，预估费用¥{total_cost:.2f}元")
