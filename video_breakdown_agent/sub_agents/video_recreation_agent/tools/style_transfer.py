...
from video_breakdown_agent.utils.doubao_client import call_doubao_vision  # 复用现有LLM调用
...
def smart_replace_keywords(
    ...
    # 如果需要，确保镜头语言保留
    if preserve_camera:
        camera_keywords = ["镜头", "特写", "中景", "全景", "环绕", "推进", "拉远", "跟拍", "手持", "固定机位"]
        # 检查镜头关键词是否仍然存在
        has_camera = any(keyword in new_prompt for keyword in camera_keywords)
        if not has_camera and any(keyword in original_prompt for keyword in camera_keywords):
            # 从原提示词中提取镜头描述并附加
...
"""
    ...
    try:
        ...
        if preserve_camera and "镜头" not in new_prompt and "镜头" in original_prompt:
            ...
            return smart_replace_keywords(original_prompt, {original_product: target_product})
